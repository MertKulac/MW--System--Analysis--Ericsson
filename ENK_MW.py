...
terkos = pd.read_csv('terkos.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])
#arsuz = pd.read_csv('arsuz.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])
#cebeci = pd.read_csv('cebeci.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])
#iztuzu = pd.read_csv('iztuzu.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])
#horasan = pd.read_csv('horasan.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])
ayazma = pd.read_csv('ayazma.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])
kefken= pd.read_csv('kefken.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])
kilyos = pd.read_csv('kilyos.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])
#harran = pd.read_csv('harran.csv',  usecols=[0,1,2,3,4,5,6,7,9,13,17,26,66,91,106])

#terkos=terkos.append(arsuz, ignore_index=True)
#terkos=terkos.append(cebeci, ignore_index=True)
#terkos=terkos.append(iztuzu, ignore_index=True)
#terkos=terkos.append(horasan, ignore_index=True)
terkos=terkos.append(ayazma, ignore_index=True)
terkos=terkos.append(kefken, ignore_index=True)
terkos=terkos.append(kilyos, ignore_index=True)
#terkos=terkos.append(harran, ignore_index=True)

wrc = pd.ExcelWriter('terkos_0.xlsx')
terkos.to_excel(wrc, sheet_name='dfs')
wrc.save()

df0= pd.read_excel('terkos_0.xlsx')
df0["Terminal_ID"]=df0["Terminal_ID"].astype(str)
dfs=df0["Terminal_ID"].str.slice(1,2,1)
df0.insert(loc=0, column='TYPE',value=dfs)

wr0 = pd.ExcelWriter('terkos_1.xlsx')
df0.to_excel(wr0, sheet_name='dfs')
wr0.save()

dft= pd.read_excel('terkos_1.xlsx')
...
wr1 = pd.ExcelWriter('terkos_mobil_check.xlsx')
